...
export const CHILD_PRIMARY_KEY_COMMENT = "This is the primary key of the child schema";
export const CHILD_ENUM_PROPERTY_OPTIONS = ["option1", "option2", "option3"];
export const CHILD_PARENT_ONE_ID_COMMENT = "This is the parent one ID in the child schema";
export const CHILD_PARENT_TWO_ID_COMMENT = "This is the parent two ID in the child schema";
export const CHILD_PARENT_NAME_WITH_NOTES_COMMENTS = ["This is a string field with notes for the parent name in the child schema", "Additional notes for the string field in the child schema"];
export const CHILD_NUMBER_PROPERTY_NOTES = ["This is a number field with notes in the child schema", "Additional notes for the number field in the child schema"];
export const CHILD_ARRAY_PROPERTY_NOTES = ["This is an array field with notes in the child schema", "Additional notes for the array field in the child schema"];
...
export const childSchema = z.object({
    ...
    parentOneId: z.string().foreignKey(parentOneSchema, "id").notes(CHILD_PARENT_ONE_ID_COMMENT),
    parentTwoId: z.string().foreignKey(parentTwoSchema, "id").notes(CHILD_PARENT_TWO_ID_COMMENT),
    parentNameWithNotes: z.string().notes(CHILD_PARENT_NAME_WITH_NOTES_COMMENTS[0]).notes(CHILD_PARENT_NAME_WITH_NOTES_COMMENTS[1]),
    stringProperty: z.string(),
    stringWithDescription: z.string(),
    numberProperty: z.number().notes(CHILD_NUMBER_PROPERTY_NOTES[0]).notes(CHILD_NUMBER_PROPERTY_NOTES[1]),
    integerProperty: z.number().int(),
    booleanProperty: z.boolean(),
    isoDateProperty: z.iso.datetime(),
    isoDurationProperty: z.iso.duration(),
    dateProperty: z.date(),
    arrayProperty: z.array(z.string().notes(CHILD_ARRAY_PROPERTY_NOTES[0]).notes(CHILD_ARRAY_PROPERTY_NOTES[1])), // Notes on array elements
