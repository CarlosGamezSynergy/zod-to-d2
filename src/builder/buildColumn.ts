import { isEnumProperty, PropertyType } from "../types/Property.type";

export function buildColumn(property: PropertyType) {
    const propNameChain = property.name.split(".");
    const propName = propNameChain.slice(1, propNameChain.length).join(".") || property.name;

    let prop = `"${propName}${property.isOptional ? "?" : ""}": `

    if (isEnumProperty(property)) {
        prop += `ENUM(${Object.values(property.values).map(value => `'${value}'`).join(", ")})`
    } else {
